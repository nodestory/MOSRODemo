...
public class MOSROReturn {
	...
	public MOSROReturn(List<MOSROCategory> cat, boolean[][] output) {
		this.category = cat;
		this.y = output;
		this.mcc = new int[category.size()];
	}
	...
	public boolean[] MostConnectedComponent(int c, int[] GridSize) {
		...
		for (int i = 0; i < len; i++) {
			if (cc[i] == 0) {
				...
				// Log.v("MOSRO", "Label#"+String.valueOf(label)+
	...
	private int BFS(int[] map, int start, int label) {
		...
		while (Q.isEmpty() == false) {
			...
			count++;
			// Log.v("MOSRO",
			// "curr="+String.valueOf(curr)+", label#"+String.valueOf(label));
			// Log.v("MOSRO", LogString(map));
			// Log.v("MOSRO",
			// ""+String.valueOf(curr%GridSize[0]+1)+","+String.valueOf(map[curr+1]));
			// Log.v("MOSRO",
			// ""+String.valueOf(curr+GridSize[0])+","+String.valueOf(map[curr+GridSize[0]]));
			...
			// bottom-right
			if ((curr % GridSize[0]) + 1 < GridSize[0]
					&& (curr + GridSize[0] < (GridSize[0] * GridSize[1]))
					&& map[curr + 1 + GridSize[0]] == 0) {
